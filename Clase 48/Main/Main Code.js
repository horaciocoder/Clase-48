...
This game was finished December 4th, 2021.
...
function StartSettings() {
    ...
    if (mouseWentDown("left") && GameStateManager.compareState(gameState, SET)|| mouseWentDown("right") && GameStateManager.compareState(gameState, SET)) {
...
function IntroSettings() {
    ...
    if (giveInstructions) {
        ...
        textAlign(CENTER);
        textSize(Math.sqrt((canvas.width + canvas.height) / 2) * 1.5);
        text("Press Space to Jump", canvas.width / 2, canvas.height / 3);
        text("Left Click to Shoot", canvas.width / 2, canvas.height / 2);
...
function GameSettings() {
    ...
    if (keyDown("space") && player.y >= (canvas.height * 0.68)) {
    ...
    if (mouseWentDown("left") && bulletActive === false) {
        spawnBullet("Player");
        bulletActive = true;
    }
    else if (mouseWentDown("left") && bulletActive === true) {
        noShootSound.play();
    }
    for (i = 0; i < obstacles.length; i++) {
